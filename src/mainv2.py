...
def main():
    ...
    for episode in range(NB_GAMES):
        ...
        #do not save before a fixed level of randomness is reached
        if agent.epsilon < 0.60:
            if (avg_score > max_avg_score):
                max_avg_score = avg_score
                last_avg_score = avg_score
                cpt_increase += 1
                #if the average score is increased 15 times, save the model, 15 is an arbitrary limit, can be adjusted if needed
                if cpt_increase > 15:
                    cpt_increase = 0
                    cpt_episode = 0
            ...
            else:#if no maximum is reached, take a score close to the maximum or take an average peak in the last 50 games
                if (((cpt_episode > 50) and ((max_avg_score - avg_score) < 0.5)) or ((cpt_episode > 15) and ((avg_score > (total_avg_score/cpt_episode)) and (avg_score > last_avg_score)))):
                    last_avg_score = avg_score
                    total_avg_score = 0
                    cpt_increase += 1
                    cpt_episode = 0
